...

{
    class MathProgramming
    {
        ...
        public static void SolveSimulInsDelSubproblem(Model m, Solution sol, int maxIns, int maxDel, bool hide_tmax_constraint = false, double custom_tmax = 0)
        {
            ...
            //Subproblems.SimulInsDelSubproblemBestVeh(m, sol, maxIns, maxDel, custom_tmax, hide_tmax_constraint);
        }

        /*
         * Solve the TSP. Fixed sets and nodes, optimize orders http://webhotel4.ruc.dk/~keld/research/LKH/
         */
        public static void SolveTSP(Model m, Solution sol, double secLimit = 2, double mipGapLimit = 1e-4, double heur = 0.5, bool hide_errors = false)
        {
            ...
            foreach (Route route in sol.routes)
            {
                if (route.nodes_seq.Count > 4)
                {
                    ...
                    TSP.LKHAlgorithm(m, route, hide_errors: hide_errors);
                    //watch.Stop();
                    //var elapsedMs = watch.ElapsedMilliseconds;
                    //Console.WriteLine("LKH: " +  elapsedMs + " ms");
