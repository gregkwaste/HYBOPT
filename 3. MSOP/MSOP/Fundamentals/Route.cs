...
namespace MSOP.Fundamentals
{
    public class Route
    {
        ...
        public Route(int id = -1)
        {
            ...
            time = 0;
            total_profit = 0;
            nodes_seq = new List<Node> { m.depot, m.depot };
            sets_included = SetsIncluded(nodes_seq);
        }

        public Route(List<Node> nodes_seq, int id = -1)
        {
            if (id == -1)
            {
                this.id = counter++;
            ...
            time = CalcTime(nodes_seq);
            total_profit = TotalProfit(nodes_seq);
        ...
        public static int CalcTime(List<Node> nodes_seq)
        {
            Model m = Model.model;
            int time = 0;
        ...
        public static int TotalProfit(List<Node> nodes_seq)
        {
            Model m = Model.model;
            int total_profit = 0;
        ...
        public bool CheckRoute() //checks if everything in route is correct, assuming that the model is correct
        {
            Model m = Model.model;
            ...
            if (!isEverythingOk)
            {
                Console.WriteLine("Error found in the route.");
            }
            return isEverythingOk;
        }

        public static List<Set> SetsIncluded(List<Node> nodes_seq)
        {
            Model m = Model.model;
